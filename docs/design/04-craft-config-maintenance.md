...
裁縫は固定基準値職人として扱い、判定は不足、通常チャンス、超過、基準値、会心時のみ確定、超過確定、超過中に分類します。
```js
normal: {
  sew: [
...
木工は固定基準値職人として扱い、判定は不足、通常チャンス、超過、基準値、会心時のみ確定、超過確定、超過中に分類します。
...
2倍削り、3倍削りなど複数回削る特技は、基礎分布を合算した後の値ごとの発生確率を表示します。
