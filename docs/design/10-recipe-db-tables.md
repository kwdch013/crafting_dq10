...
設計方針と原案からの変更点は [レシピDB設計](./09-recipe-db-schema.md) を参照します。
...
木工の盤面は3行2列のため、`exist_c`、`exist_f`、`exist_i` は常に `false` です。
...
| 列 | 論理名 | 型 | 必須 | unique | 説明 |
| --- | --- | --- | --- | --- | --- |
| `id` | ID | integer | o | o | 主キー。`craft_master.id` への外部キー |
...
| 各分類テーブル | `category_id = 0` (未分類) と、現行データの大項目 |
