...
鍛冶配置のマスを右クリックすると、現在値を直接編集できます。固定状態は扱いません。
BOARDセルの判定は1倍相当で表示し、光地金の光選択はノード上でも切り替えられます。鍛冶では固定文言を表示せず、会心確定は本会心！、非会心最大で基準範囲に入る場合はゲージ突入と表示します。戻りは200n+50℃で次が戻りターンであることを表示し、戻り値は手動入力します。
