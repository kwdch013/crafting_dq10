...
- 200℃単位の温度(200℃〜2000℃)ごとに、威力別の会心確定ライン(会心最小値)を `会心確定 n ≦ ◯` として一覧表示します。`n` は基準値までの残り数値で、見出し直下に `n = 基準値までの残り数値` と注記します。
- あわせて、非会心の最大ダメージでも基準値へ届かなくなる境界（非会心最大+1）を `非会心不足 n ≧ ◯（非会心最大 ◯）` として同じ枠内に併記します。会心が必須になる残り数値の下限を示すための表示です。
