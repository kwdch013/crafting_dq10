...
初期位置は `initialGridCell`、現在位置は `gridCell` に保持し、BOARD表示上の座標ラベルは出さずにDOMの `data-initial-row`、`data-initial-column`、`data-current-row`、`data-current-column` で取得できるようにします。
