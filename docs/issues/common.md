...
### #230 共通: 保存失敗時のalert文言をエラーの種類に応じて分ける（2026-09-02 close済み）

`app/main.js` の `persistRecipeToApi` / `deleteRecipeFromApi` がレスポンス本文を読まずステータスコードだけで例外を投げていたため、保存・削除の失敗が理由を問わず同じ文言になっていました。PR #229 で保存先を PostgreSQL に切り替えられるようにしたところ、DB の `UNIQUE (class, name)` により同じ職人で同名のレシピを保存すると `400 recipe_name_already_exists` が返るようになり、名前が重複しているだけなのに「APIの起動状態を確認してください」と表示される状態でした。本文の `error` 識別子とステータスを `Error` へ持たせ、識別子ごとの説明文を選ぶよう変更しました。4xx で既知の識別子なら内容を説明し、5xx と通信失敗は従来どおりAPIの起動状態を案内します。未知の識別子はサーバー由来の文字列を画面へ出さず既定文言にします (`api/main.py` の `except` 順序により、不正なJSONではPythonのパーサー文言が返るため。#227 参照)。あわせて利用者へ見せる文言から内部のファイル名を外し「サーバーへの保存 / サーバーからの削除」としました。`RECIPE_STORE=postgres` では保存先がDBであり `recipes.json` では事実と異なるためです。保存失敗時にブラウザへ保存する挙動は変えておらず、真実源をDBへ反転させるのは #220 (段階3c) の範囲です。PR #234 で dev へ merge 済み。URL: https://github.com/kwdch013/crafting_dq10/issues/230
