...
- UIは基本設定の変更以外でボタンやBOARDの位置が動かないようにします。温度、特性発動、状態表示など作業中に頻繁に変わる値で表示有無が切り替わる場合は、非表示時も同等の表示領域を確保し、文字は通常状態や未発動状態として表示します。
- 表示有無で高さが変わる領域は、非表示時も同等の表示領域を確保し、`hidden` や `display: none` は職人種別や画面モードなど基本設定の変更に伴う切替に限定します。
