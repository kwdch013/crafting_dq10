...
CREATE TABLE craft_master (
	...
	-- 職人別テーブルからの識別子付き外部キーの参照先です。
	CONSTRAINT craft_master_id_class_unique UNIQUE (id, class),
	-- 並び順の重複を防ぎます。並べ替え中の一時的な重複を許すため遅延評価にします。
	CONSTRAINT craft_master_sort_order_unique UNIQUE (class, sort_order) DEFERRABLE INITIALLY DEFERRED
);
...
CREATE TABLE tool_category (
	...
	row_a smallint CHECK (row_a BETWEEN 1 AND 4),
	col_a smallint CHECK (col_a BETWEEN 1 AND 2),
	row_b smallint CHECK (row_b BETWEEN 1 AND 4),
	col_b smallint CHECK (col_b BETWEEN 1 AND 2),
	row_c smallint CHECK (row_c BETWEEN 1 AND 4),
	col_c smallint CHECK (col_c BETWEEN 1 AND 2),
	row_d smallint CHECK (row_d BETWEEN 1 AND 4),
	col_d smallint CHECK (col_d BETWEEN 1 AND 2),
	row_e smallint CHECK (row_e BETWEEN 1 AND 4),
	col_e smallint CHECK (col_e BETWEEN 1 AND 2),
	row_f smallint CHECK (row_f BETWEEN 1 AND 4),
	col_f smallint CHECK (col_f BETWEEN 1 AND 2),
	row_g smallint CHECK (row_g BETWEEN 1 AND 4),
	col_g smallint CHECK (col_g BETWEEN 1 AND 2),
	row_h smallint CHECK (row_h BETWEEN 1 AND 4),
	col_h smallint CHECK (col_h BETWEEN 1 AND 2),
